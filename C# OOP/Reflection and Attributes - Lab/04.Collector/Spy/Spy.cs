...
namespace Stealer
{
    public class Spy : ISpy
    {
        ...
        {
            Type classType = Type.GetType(className);
            MethodInfo[] classGettSet = classType
                .GetMethods(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);

            StringBuilder stringBuilder = new StringBuilder();

            foreach (MethodInfo getter in classGettSet.Where(g => g.Name.StartsWith("get")))
            {
                stringBuilder.AppendLine($"{getter.Name} will return {getter.ReturnType}");
            }

            foreach (MethodInfo setter in classGettSet.Where(s => s.Name.StartsWith("set")))
            {
